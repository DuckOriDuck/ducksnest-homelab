{ lib, config, ... }:
{
  ...
  certToolkit.owningHostKey =
    let
      keyFileName = {
        "laptop-old" = "ducksnest_cert_mng_key_old.pub";
        "laptop-ultra" = "ducksnest_cert_mng_key_ultra.pub";
        "laptop-firebat" = "ducksnest_cert_mng_key_firebat.pub";
        "ec2-controlplane" = "ducksnest_cert_mng_key_ec2.pub";
      }.${config.networking.hostName} or "ssh_host_ed25519_key.pub";
    in
      "./ssh-host-keys/${config.networking.hostName}/${keyFileName}";

  # Default key configuration
  certToolkit.defaults.key = {
    algo = "rsa";
    size = 2048;
  };
  ...
  # All certificates under single CA
  certToolkit.cas.k8s.certs = {
    # etcd certificates
    etcd-server = {
      commonName = "etcd";
